...
import org.apache.nutch.parse.amisfinder.CopainsdavantTest.EXTRACT_METHOD;
...
public abstract class AbstractCrawlTest extends TestCase {
	 ...
	 * @param extract the extract method to use
	 * @param filePath
	 * @param url
	 * @return the resulting content after the crawl
	 */
	protected ParseResult simulateCrawl(PARSER parseFilter, EXTRACT_METHOD extract, String filePath, String url) {
		...
		try {
			...
			HtmlParseFilter filter = null;
			if (extract == EXTRACT_METHOD.XSLT_EXTRACTION) {
				filter = new AmisfinderParseFilterXsl();
			} else {
				filter = new AmisfinderParseFilter();
			}
