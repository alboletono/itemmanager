 ...
 * 
 */
public class AmisfinderParseFilterXsl implements HtmlParseFilter {
	...
	public ParseResult filter(Content content, ParseResult parseResult, HTMLMetaTags metaTags, DocumentFragment document) {
		...
		try {
			...
			if (LOG.isDebugEnabled()) {
				...
				if (debugFile != null)
					AmisfinderParseFilterXsl.saveDOMOutput(result.getNode(), new File(debugFile));
			}

			AmisfinderParseFilterXsl.updateMetadata(result.getNode(), parse);
	...
	protected static void updateMetadata(Node node, Parse data) {
		...
		for (TDocument document : documents.getDocument()) {
			...
			for (TContentMeta content : document.getContentMeta()) {
				data.getData().getParseMeta().add(content.getName(), content.getValue());
			}
		}
